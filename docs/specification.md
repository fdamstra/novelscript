...
2. **One way to branch** — Pattern matching is the sole branching mechanism. There is
   no `if/else` statement, no `switch`, no `cond`. This eliminates choice paralysis
   for code generators and ensures exhaustive case handling.
...
8. **Explicit over implicit** — No hidden conversions, no implicit coercions. Every
   value is produced by an expression; the last expression in a block is the result.
...
`stream` and `collect` are built-in functions, not keywords. They use standard
function-call syntax but are provided by the runtime.
...
The `>>` operator composes bijections (see Section 9.3) and has lower precedence
than all of the above, used only in binding expressions.
...
floats when necessary. Integer division is performed with `//` (floor division),
while `/` is true division.

### 3.2 Composite Types

| Type           | Description                         | Literal / Constructor             |
|----------------|-------------------------------------|-----------------------------------|
| `(A, B, ...)`  | Tuple (fixed-size, heterogeneous)   | `(1, "hello")`, `(a, b, c)`      |
...
Tuples are fixed-length, ordered collections of possibly different types. They are
constructed with parentheses and can be destructured in patterns and lambda
parameters: `(a, b) => a + b`.
...
Bindings can destructure values using any pattern from Section 6.3:
...
The `mut` modifier is only valid with a simple identifier pattern.
...
The `return` keyword exits the enclosing function immediately with the given value:

```
fn find_index(items: List[Text], target: Text) -> Option[Num] do
  items |> enumerate |> each((i, item) =>
    match item == target
      case true => return some(i)
      case false => ()
    end
  )
  none
...
`return` applies to the nearest enclosing `fn`, not to lambdas or `do` blocks.
...
block. The parameters are delimited with `|...|`:
...
- `ensure` — Checked after the body executes. The special name `result` refers to
  the return value.
...
| `[head, ..]`             | List with at least 1 element, rest discarded |
...
The built-in `tap` function executes a side effect without altering the pipeline
value:
...
The built-in `inspect` function prints a debug representation of the value and
...
`with` is special syntax, not a regular function. Its arguments use `field_name:
value` syntax (like shape construction) rather than positional arguments. It can be
used in pipelines or called directly: `with(alice, age: 31)`.

### 8.5 Shape Methods

Functions can be associated with a shape by defining them inside a module of the
same name:
...
The interpreter can optionally verify that forward and reverse are inverses by
running round-trip checks on proof values:
...
Proof blocks are inline test assertions. They execute as part of the program and
abort on failure. They can be disabled in production with an interpreter flag
(`--no-proofs`).
...
Stream operations such as `map`, `filter`, `take`, `each`, `reduce`, `join`, and
others work on both lists and streams. When called on a list, the list is implicitly
converted to a stream. Terminal operations that produce a collection (like `collect`)
return a `List`.
...
stream |> append(value)      -- append an element to end of stream
...
upper(t: Text) -> Text
lower(t: Text) -> Text
upper_first(t: Text) -> Text         -- capitalize first character
trim(t: Text) -> Text
...
```
unwrap(o: Option[T]) -> T                      -- panics on none
unwrap(r: Result[T, E]) -> T                   -- panics on err
unwrap_or(o: Option[T], default: T) -> T       -- returns default on none
unwrap_or(r: Result[T, E], default: T) -> T    -- returns default on err
unwrap_or_else(o: Option[T], fn: () -> T) -> T
unwrap_or_else(r: Result[T, E], fn: () -> T) -> T
```
...
compose(f, g) -> fn                            -- compose: compose(f, g)(x) = f(g(x))
...
stream(init: T, fn: (T) -> T) -> Stream[T]    -- infinite stream from seed + step
collect(s: Stream[T]) -> List[T]              -- materialize stream into list
```
...
A NovelScript program consists of one or more `.ns` files. Execution begins at the
top of the main file and proceeds top-to-bottom. Declarations (functions, shapes,
modules, bijections) are hoisted — they can be used before they are textually
defined.
...
fn_decl          ::= annotation* 'fn' IDENT type_params? '(' params ')'
                     ('->' type)? contract* 'do' block 'end'
...
type             ::= dotted_name type_args?
                   | '(' type (',' type)+ ')'
                   | '(' params ')' '->' type
type_args        ::= '[' type (',' type)* ']'

expression       ::= pipeline
pipeline         ::= conditional (('|>' | '|?>') pipe_target)*
pipe_target      ::= dotted_name call_args?
                   | dotted_name trailing_block
                   | with_expr
...
comp_op          ::= '==' | '!=' | '<' | '>' | '<=' | '>='
range_expr       ::= addition (('to' | 'until') addition)?
addition         ::= multiplication (('+' | '-' | '++') multiplication)*
multiplication   ::= unary (('*' | '/' | '//' | '%') unary)*
unary            ::= ('-' | 'not') unary | postfix
postfix          ::= primary ('.' IDENT | '[' expression ']' | call_args)*
primary          ::= literal | IDENT | '(' expression ')' | block_expr
                   | match_expr | lambda | list | map | tuple | void_lit
                   | with_expr

literal          ::= NUM | STRING | 'true' | 'false' | 'none'
void_lit         ::= '(' ')'
...
pattern          ::= literal | IDENT | '_'
                   | list_pattern | map_pattern | shape_pattern
                   | option_pattern | result_pattern | tuple_pattern
list_pattern     ::= '[' (pattern (',' pattern)* ('..' IDENT?)?)? ']'
map_pattern      ::= '#{' STRING ':' pattern (',' STRING ':' pattern)* '}'
shape_pattern    ::= IDENT '{' IDENT ':' pattern (',' IDENT ':' pattern)* '}'
option_pattern   ::= 'some' '(' pattern ')' | 'none'
result_pattern   ::= 'ok' '(' pattern ')' | 'err' '(' pattern ')'
tuple_pattern    ::= '(' pattern ',' pattern (',' pattern)* ')'

lambda           ::= param_list '=>' expression
                   | param_list '=>' 'do' block 'end'
trailing_block   ::= 'do' '|' params '|' block 'end'
param_list       ::= IDENT | '(' params ')' | '(' ')'

with_expr        ::= 'with' '(' (expression ',')? with_arg (',' with_arg)* ')'
with_arg         ::= IDENT ':' expression
